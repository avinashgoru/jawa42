...
/**
 * Utility to send booking & inquiry confirmation emails via Nodemailer from Next.js
 */
export const sendBookingEmail = async (lead) => {
  ...
  try {
    ...
    if (email) {
      await transporter.sendMail({ from: emailFrom, to: email, subject: customerSubject, html: customerHtml });
    }
    await transporter.sendMail({ from: emailFrom, to: adminEmail, subject: adminSubject, html: adminHtml });
